...
#df = read.csv(here('data', 'FINALCohort_13Aug2019.csv'))
names(df)
...
# ## eligible dates ##
# dates_fix = lapply(1:ncol(dates), function(x) fix_date(dates[,x]))
# valid_dates = sapply(dates_fix, function(x) x >= "2017-05-22" & x <= "2019-05-31")
# num_visits = rowSums(valid_dates, na.rm = T)
# head(dates_fix[[1]])
# head(dates)
# head(valid_dates)
# lapply(valid_dates,table)
# 
# ## eligible dates
# 
## del_by_us: gestation by us
##ptb calc
...
df_24 = df %>% filter(anc1ga_anc <= 24 & number_anc >= 2)
#df_16 = df %>% filter(anc1ga_anc <= 16 & number_anc >= 2)
