...
date_eligibility = function(date, upper) {
  
  #2.       For all eligible women, please make HC specific lists (for all events) 
  #of women with invalid dates (< DATE.MDY(05,22,2017), > DATE.MDY(2,14,2019). 
  is_eligible = date < "2017-05-22" | date > upper
  return(is_eligible)
}
...
make_table = function(df, arm, day) {
  ...
  if (arm == 1) {
    ...
    anc1_enroll_compl_a1 = sum(j$enrollment_complete >= 0, na.rm = T)/nrow(j)
    has_anc1_or_enroll = sum(!is.na(j$anc1date_anc) | !is.na(j$enroll_date), na.rm = T)/nrow(j)
    ## anc1date_anc is filled in as well | enroll_date filled in eligible for anc1
    
    #if they filled out 100 vars in enroll form then enroll = 2
    #if filled out 50 vars then incomplete but = 1 (may have completed sufficient info)
    ...
    has_at_least_1_fuvst = rowSums(!is.na(ancfuvst_date), na.rm = T)
    
    datefile_fuvst = j[, grep('datefile_fuvst', names(j), value = T)]
    lapply(datefile_fuvst, function(x) (mean(!is.na(x))))
    ...
    num_complete_anc2_lt_24_wks_visit2 = sum(j$anc1ga_anc < 24 & !is.na(j$date_ancfuvst_2), na.rm = T)
    num_complete_anc2_lt_24_wks = sum(j$anc1ga_anc < 24 & has_at_least_1_fuvst > 0, na.rm = T)

    # delivered_a1 
    # 
    pct_complete_anc2_lt_24_wks_visit2 = num_complete_anc2_lt_24_wks_visit2 / num_lt_24_wks
    ...
    num_complete_anc2_lt_24_wks_del_visit2 = sum(j$anc1ga_anc < 24 & !is.na(j$date_ancfuvst_2)
                                                                & !is.na(j$date_delivery_mat), na.rm = T)
    num_complete_anc2_lt_24_wks_del = sum(j$anc1ga_anc < 24 & has_at_least_1_fuvst > 0
                                                 & !is.na(j$date_delivery_mat), na.rm = T)
    

    num_expected_anc2_lt_24_wks_del = sum(j$expected2 == T & (edd.asdate < day) & j$anc1ga_anc < 24, na.rm = T)
 
    out = rbind(num.enroll, anc1_enroll_compl_a1, has_anc1_or_enroll, anc1_date_compl_a1, 
                ...
                expected_pnc_1,
                num_lt_24_wks, pct_lt_24_wks, num_complete_anc2_lt_24_wks_visit2, num_complete_anc2_lt_24_wks,
                pct_complete_anc2_lt_24_wks_visit2, pct_complete_anc2_lt_24_wks,
                num_expected_anc2_lt_24_wks,
                num_complete_anc2_lt_24_wks_del_visit2, num_complete_anc2_lt_24_wks_del,
                num_expected_anc2_lt_24_wks_del)
